...
def create_question_without_choice(question_text, days):
    ...
    given number of `days` offset to now (negative for questions published
    in the past, positive for questions that have yet to be published)
    Creates the question without any choices.
    """
    time = timezone.now() + datetime.timedelta(days=days)
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_question_with_no_choices(self):
        """
        Questions with no choices aren't displayed on the index page.
        """
        create_question_without_choice(question_text="Choiceless Question", days=0)
        response = self.client.get(reverse("polls:index"))
        self.assertContains(response, "No polls are available.")
        self.assertQuerySetEqual(response.context["latest_question_list"], [])
